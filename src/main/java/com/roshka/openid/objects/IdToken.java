...
public class IdToken {
    ...
    // REQUIRED. Issuer Identifier for the Issuer of the response. The iss value is a
    ...
    // REQUIRED. Subject Identifier. A locally unique and never reassigned identifier within
    // the Issuer for the End-User, which is intended to be consumed by the Client, e.g.,
    ...
    // REQUIRED. Audience(s) that this ID Token is intended for. It MUST contain the OAuth
    // 2.0 client_id of the Relying Party as an audience value. It MAY also contain identifiers
    ...
    // processing. The processing of this parameter requires that the current date/time
    // MUST be before the expiration date/time listed in the value. Implementers MAY provide
    ...
    // Request to the ID Token. If present in the ID Token, Clients MUST verify that the
    // nonce Claim Value is equal to the value of the nonce parameter sent in the Authentication
    // Request. If present in the Authentication Request, Authorization Servers MUST include
    // a nonce Claim in the ID Token with the Claim Value being the nonce value sent in
    // the Authentication Request. Authorization Servers SHOULD perform no other processing
    // on nonce values used. The nonce value is a case sensitive string.
    private String nonce;

    // OPTIONAL. Authentication Context Class Reference. String specifying an Authentication
    // Context Class Reference value that identifies the Authentication Context Class that
    // the authentication performed satisfied. The value "0" indicates the End-User authentication
    // did not meet the requirements of ISO/IEC 29115 [ISO29115] level 1. Authentication
    // using a long-lived browser cookie, for instance, is one example where the use of
    // "level 0" is appropriate. Authentications with level 0 SHOULD NOT be used to authorize
    // access to any resource of any monetary value. (This corresponds to the OpenID 2.0
    // PAPE [OpenID.PAPE] nist_auth_level 0.) An absolute URI or an RFC 6711 [RFC6711] registered
    // name SHOULD be used as the acr value; registered names MUST NOT be used with a different
    // meaning than that which is registered. Parties using this claim will need to agree
    // upon the meanings of the values used, which may be context-specific. The acr value
    // is a case sensitive string.
    private String acr;

    // OPTIONAL. Authentication Methods References. JSON array of strings that are identifiers
    ...
    // Token has a single audience value and that audience is different than the authorized
    // party. It MAY be included even when the authorized party is the same as the sole
    // audience. The azp value is a case sensitive string containing a StringOrURI value.
    private String azp;

    // OPTIONAL. Access Token hash value. Its value is the base64url encoding of the
    // left-most half of the hash of the octets of the ASCII representation of the access_token
